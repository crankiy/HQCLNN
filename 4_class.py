...
def load_data():
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\4_class_mnist\mnist_0-3_train28_24000.npz'
    with np.load(path, allow_pickle=True) as f:
        x_train, y_train = f['image'], f['label']
    f.close()  # 关闭文件
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\4_class_mnist\mnist_0-3_test28_4000.npz'
    with np.load(path, allow_pickle=True) as f:
...
y_train_q = tf.one_hot(Y_train, depth=4)
y_test_q = tf.one_hot(Y_test, depth=4)
n_sample = 600
test_sample = 100
    ...
    for i in range(len(circuit_number)):
        ...
        for k in range(8, 16):
            if index[k] > 0:
                ...
                w_num = w_x2_num+1
    out = K.stack([K.real(c_all.expectation([tc.gates.x(), [i]])) for i in range(4, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.y(), [i]])) for i in range(4, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.z(), [i]])) for i in range(4, nqubits)]
                  )
    return out
...
def train(epochs, circuit_path):
    ...
    qml = qml_layer(flatt)
    output = tf.keras.layers.Dense(4, activation="softmax")(qml)
...
def evolution_next_path():
    ...
    print(max_acc_path)  # [0.44, 0.39, 0.31, 0.4, 0.25, 0.43]  #输出每条路径的最大准确率
    print(max_loss_path)
    print(para_path)
    max_path_num = max_acc_path.index(max(max_acc_path))  # 最大准确率路径的索引
    fro_path = population_initial[max_path_num]  # 最大准确率的路径
    next_path = generate_next_population(fro_path, 2, population_size_2)  # 基于最大准确率路径，新增2深度的多条路径
