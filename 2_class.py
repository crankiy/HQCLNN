...
def load_data():
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\2_class_mnist\mnist_01_train28_12000.npz'
    with np.load(path, allow_pickle=True) as f:
        x_train, y_train = f['image'], f['label']
    f.close()
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\2_class_mnist\mnist_01_test28_2000.npz'
    with np.load(path, allow_pickle=True) as f:
        x_test, y_test = f['image'], f['label']
    f.close()
...
y_train_q = tf.one_hot(Y_train, depth=2)
y_test_q = tf.one_hot(Y_test, depth=2)
n_sample = 12000
test_sample = 2000
    ...
    for i in range(len(circuit_number)):
        ...
        for k in range(8, 16):
            if index[k] > 0:
                ...
                w_num = w_x2_num+1
    out = K.stack([K.real(c_all.expectation([tc.gates.x(), [i]])) for i in range(7, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.y(), [i]])) for i in range(7, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.z(), [i]])) for i in range(7, nqubits)]
                  )
    return out
...
def train(epochs, circuit_path):
    ...
    qml = qml_layer(flatt)
    output = tf.keras.layers.Dense(2, activation="softmax")(qml)
    model = tf.keras.Model(inputs=inputs, outputs=output)
    for layer in model.layers:
        if not isinstance(layer, (tc.keras.QuantumLayer, tf.keras.layers.Dense)):
...
class evolution_next_maxacc_path:
    def __init__(self, current_mult_path):
        ...
        for i in range(len(circuit_number)):
            ...
            for k in range(8, 16):
                if index[k] > 0:
                    ...
                    c_all.RX(index[k] - 1, theta=weights[0][w_x2_num])
                    w_num = w_x2_num + 1
        out = K.stack([K.real(c_all.expectation([tc.gates.x(), [i]]))for i in range(6,nqubits)]
                      + [K.real(c_all.expectation([tc.gates.y(), [i]]))for i in range(6,nqubits)]
                      + [K.real(c_all.expectation([tc.gates.z(), [i]]))for i in range(6,nqubits)])
        return out
    ...
    def evolution_class(self):
        ...
        for i in range(population_size_2):
            loss, acc, val_loss, val_acc, model_par = evolution_next_maxacc_path.train_class(self, 10, i)
            max_acc = max(acc)
            max_acc_index = acc.index(max(acc))
            max_acc_path.append(max_acc)
            para_path.append(model_par)
...
def main():
    ...
    evo_net_path = evolution_next_maxacc_path(a)
    for i in range(100):
