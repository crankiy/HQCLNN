...
def load_data():
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\6_class_mnist\mnist_0-5_train28_36000.npz'
    with np.load(path, allow_pickle=True) as f:
        x_train, y_train = f['image'], f['label']
    f.close()
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\6_class_mnist\mnist_0-5_test28_6000.npz'
    with np.load(path, allow_pickle=True) as f:
...
y_train_q = tf.one_hot(Y_train, depth=6)
y_test_q = tf.one_hot(Y_test, depth=6)
n_sample = 600
test_sample = 100
...
def generate_initial_path(path_length):
    initial_code_1 = np.random.randint(0, const1, 1)  # 123382
    ...
    for i in range(len(circuit_number)):
        ...
        for k in range(8, 16):
            if index[k] > 0:
                ...
                w_num = w_x2_num+1
    out = K.stack([K.real(c_all.expectation([tc.gates.x(), [i]])) for i in range(2, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.y(), [i]])) for i in range(2, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.z(), [i]])) for i in range(2, nqubits)]
                  )
    return out
...
def train(epochs, circuit_path):
    ...
    qml = qml_layer(flatt)
    output = tf.keras.layers.Dense(6, activation="softmax")(qml)
