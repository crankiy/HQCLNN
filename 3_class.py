...
def load_data():
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\3_class_mnist\mnist_0-2_train28_18000.npz'
    with np.load(path, allow_pickle=True) as f:
        x_train, y_train = f['image'], f['label']
    f.close()
    path = r'E:\Py_project\HQCLNN\MNIST_newdata\3_class_mnist\mnist_0-2_test28_3000.npz'
    with np.load(path, allow_pickle=True) as f:
...
y_train_q = tf.one_hot(Y_train, depth=3)
y_test_q = tf.one_hot(Y_test, depth=3)
n_sample = 18000
test_sample = 3000
    ...
    for i in range(len(circuit_number)):
        ...
        for k in range(8, 16):
            if index[k] > 0:
                ...
                w_num = w_x2_num+1
    out = K.stack([K.real(c_all.expectation([tc.gates.x(), [i]])) for i in range(5, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.y(), [i]])) for i in range(5, nqubits)]
                  + [K.real(c_all.expectation([tc.gates.z(), [i]])) for i in range(5, nqubits)]
                  )
    return out
...
def train(epochs, circuit_path):
    ...
    qml = qml_layer(flatt)
    output = tf.keras.layers.Dense(3, activation="softmax")(qml)
